            ...
            <div class="card mb-4">
                ...
                <div class="table-responsive p-0">
                    ...
                    <thead>
                        ...
                        <th class="text-center text-uppercase text-secondary text-x font-weight-bolder opacity-7">Floor</th>
                        ...
                        <th class="text-center text-uppercase text-secondary text-x font-weight-bolder opacity-7">List Item</th>
                        ...
                        @foreach ($rooms as $room)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <p class="align-middle text-center text-sm">
                                            <ul>
                                                <li>Adult : {{ $latestTransaction->TransactionSewaRoom->total_orang_dewasa }}</li>
                                                <li>Child : {{ $latestTransaction->TransactionSewaRoom->total_anak ?? 0 }}</li>
                                            </ul>
                                        </p>
                                    </td>
                                    <td>
                                    ...
                                    <div class="row justify-content-center">
                                        <div class="col-md">
                                            <div class="button-container">
                                                ...
                                                id="show-user"
                                                data-url="{{ route('checkin.booking', $latestTransaction->id ?? 0) }}"
                                                class="custom-button mt-2">Check In
                                                </a>
                                            </div>
                                            <div class="button-container">
                                                <form onsubmit="return confirm('Apakah yakin untuk menghapus data tersebut??');" action="{{ route('room.destroy', $room->id) }}" method="POST">
                                                    @method('DELETE')
                                                    @csrf
                                                    <button class="custom-button-delete mt-2">Delete</button>
                                                </form>
                                            </div>
                                        </div>
                                    </div>
                                    @elseif ($statusTransaction == 0)
                                        <div class="button-container mt-2">
                                            <a href="{{ route('booking', $room->id) }}" class="custom-button-booking mt-2">
                                                Booking
                                            </a>
                                        </div>
                                    @elseif ($statusTransaction == 1)
                                ...
                                <div class="modal-dialog">
                                    ...
                                    <form method="POST" action="/front-office/reservation/pay-transaction/{{ $latestTransaction->id ?? 0 }}">
                                        @csrf
        ...
        $('body').on('click', '#show-user', function () {
          var userURL = $(this).data('url');
            $.get(userURL, function (data) {
                ...
                // Assuming 'data' is an object containing the values
                $('#payment_paid').text(formatToIDR(data.payment_paid));
                $('#total_payment_transaction').text(formatToIDR(data.total_payment_transaction));

                if(unpaid > 0) {
                    $('#payment_unpaid').text(formatToIDR(unpaid));
                    $('#colum_input').html(`
